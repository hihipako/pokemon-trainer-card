...
/* ────────────────────────── 트레이너 칭호(트레이너군)
   ...
   8·9세대는 여기에 적어 둔다. */
/* 위키 목록이 옛 이름·붙여쓰기로 적어 둔 것들을 게임의 한국 명칭으로 바로잡는다.
   (모두 위키에서 해당 이름으로 넘겨주기가 걸려 있고, 공식 트레이너군 표와도 일치한다) */
const TRAINER_RENAME = {
  "중": "수행자",                 // ぼうず / Sage
  "기도사": "주술사",
  "수리공": "전기 작업원",
  "괴짜 연구원": "연구원",
...
/* 트레이너군이 아닌 것들 — 조직 이름 자체와, 위키에서 특정 인물 문서로
   넘어가는 1인용 직책. (위키 목록이 이런 것까지 함께 싣고 있다) */
const TRAINER_DROP = new Set([
  // 조직 이름
  "로켓단", "스컬단", "에테르재단", "플레어단", "플라스마단", "배틀샤틀레느",
  // 조직 수장 — 실제로는 한 사람
  "보스", "수수께끼 청년", "마그마단 보스", "아쿠아단 보스", "갤럭시단 보스",
  "플레어단보스", "스컬단보스", "스컬단간부", "에테르대표", "에테르지부장",
  // 시설장·기타 1인 직책
  "서브웨이마스터", "전승자", "비밀기지마스터", "루네민족", "황보가문",
  // 배틀샤토 귀족 칭호 — 공식 트레이너군 표에 없다
  "그랜드 더치스", "그랜드 듀크", "더치스", "듀크", "얼", "카운터스",
  "배런", "배러니스", "바이카운트", "바이카운터스", "마퀴스", "마르키즈",
]);
...
async function buildTrainerRoles(typeList) {
  ...
  for (const line of wiki.split("\n")) {
    ...
    if (m && cur) {
      const raw = m[1];
      const piped = raw.includes("|");
      const target = (piped ? raw.split("|")[0] : raw).trim();
      const name = (piped ? raw.split("|")[1] : raw).trim();
      // [[하인즈|돔슈퍼스타]] 처럼 인물 문서를 가리키면 그 사람만의 직책이다.
      // [[포켓몬 레인저 (트레이너)|포켓몬 레인저]] 같은 동음이의 처리는 통과시킨다.
      if (piped && !target.startsWith(name)) continue;
      if (TRAINER_DROP.has(name)) continue;
      cur.items.push(TRAINER_RENAME[name] || name);
    }
  }
